...
const LINEUP = (() => {
  ...
  // The short list of BRs where an armor lineup is genuinely worth playing,
  // and THE one nation that owns each of them. Ground mains only (no SPAA /
  // aircraft / helicopters / light tanks), tech-tree only so anyone can grind
  // the lineup, vehicles within one BR step (0.3) of the target — no swapping.
  //
  // Matchmaker-aware: a lineup at BR T fights guns from T-1.0 to T+1.0, so
  // every vehicle's armor is tested against the penetration actually present
  // in that bracket (all sources — premiums shoot at you too). A vehicle that
  // can't stand up to the bracket's guns doesn't qualify as an armor pick no
  // matter how it ranks inside its own nation, and a BR where no nation has
  // three qualifying vehicles simply isn't listed: at that rating, armor is
  // not a strategy. Near-identical lineups at adjacent BRs collapse into the
  // one with the better matchmaking spot.
  //
  ...
  // composite arrays + ERA + spall liner). Blended 45/55 hull/turret so a
  // monster turret on a paper hull (Challenger 2) can't fully hide the hull.
  // Internal scale only — never displayed as a millimeter figure.
  function bestArmorLineups(units, mode) {
    const SIZE = 5;         // most vehicles a lineup will list
    const MIN = 2;          // fewer than this isn't a lineup — nation skipped
    const AT_BR = 0.3;      // one matchmaker step: "at its BR", not a downtier pile
    const UPTIER = 1.0;     // matchmaker spread the armor must face
    // A vehicle is listed ONLY if its armor holds the advantage against the
    // bracket's guns. No fillers: the lineup is exactly the vehicles that
    // qualify, even if that's only two of them. 0.8 (not 1.0) because gunPen
    // is everyone's BEST spaded shell at a flat 1km hit — mostly estimates
    // that run hot — while real bounces come with angle and range on top.
    // Measured against known ground truth: famous walls (4.0 T-34 = 0.86,
    // 6.7 Tiger II = 0.89, Jumbo = 0.84) sit above 0.8; fillers (Tiger I at
    // 5.7 = 0.59, T-34-85 = 0.53, T26E5 at 6.7 = 0.76) sit below.
    const HOLD = 0.8;
    const armorValue = u => {
      if (u.armorEffHull != null || u.armorEffTurret != null) {
        // Casemate TDs are one plate: their superstructure lands on the
        // "turret" path and the vestigial hull would drag the blend down
        // (Ferdinand's 200mm face scored as 155). Best path for TDs; the
        // hull/turret blend is for tanks that actually have both.
        if (u.cls === "td") return Math.max(u.armorEffHull ?? 0, u.armorEffTurret ?? 0);
        return 0.45 * (u.armorEffHull ?? 0) + 0.55 * (u.armorEffTurret ?? 0);
      }
      return u.armorEff ?? Math.max(u.armorHull ?? 0, u.armorTurret ?? 0);
    };

    // Lineup candidates: real armor classes only. A light tank is never an
    // armor pick, whatever its score says.
    const mains = units.filter(u =>
      u.type === "tank" && u.cls !== "spaa" && u.cls !== "light" &&
      !u.premium && !u.squadron && !u.gift &&
      u.br[mode] != null && armorValue(u) > 0);

    // Threat guns: everything that can shoot at this lineup in its bracket —
    // every class, every source (premium/squadron/event vehicles are enemies
    // too). gunPen is the factual/estimated pen at ~1km from gunstats.json.
    const guns = units.filter(u =>
      u.type === "tank" && u.cls !== "spaa" &&
      u.br[mode] != null && u.gunPen > 0);
    ...
    for (const target of brSteps) {
      // Representative gun the wall must bounce: the median pen of the full
      // [T-1, T+1] bracket. Downtier and uptier guns weigh in equally — an
      // uptier heavy-hitter can force its way through any wall, so the
      // question is whether the armor beats the TYPICAL gun it meets.
      const pens = guns
        .filter(u => u.br[mode] >= target - UPTIER - 1e-9 && u.br[mode] <= target + UPTIER + 1e-9)
        .map(u => u.gunPen)
        .sort((a, b) => a - b);
      if (!pens.length) continue;
      const threat = pct(pens, 0.5);
      ...
      for (const nation of nations) {
        const pool = mains.filter(u =>
          ...
          armorValue(u) >= threat * HOLD);   // each pick must survive the bracket
        ...
        const picks = rankBy(pool, armorValue).slice(0, SIZE);
        ...
        const mean = picks.reduce((s, u) => s + armorValue(u), 0) / picks.length;
        const score = mean * (0.9 + 0.1 * picks.length / SIZE);
        if (!best || score > best.score) {
          best = { nation, picks, score, ratio: mean / threat };
        }
      }
      if (best) out.push({ br: target, nation: best.nation, slots: best.picks, ratio: best.ratio });
    }

    // Collapse near-identical consecutive winners (same nation, mostly the
    // same tanks one step apart): keep the BR where the armor holds up best —
    // usually the lower one, since it dodges the harder uptier bracket.
    const deduped = [];
    for (const e of out) {
      const prev = deduped[deduped.length - 1];
      if (prev && prev.nation === e.nation) {
        const ids = new Set(prev.slots.map(u => u.id));
        const shared = e.slots.filter(u => ids.has(u.id)).length;
        if (shared / Math.min(e.slots.length, prev.slots.length) >= 0.6) {
          if (e.ratio > prev.ratio) deduped[deduped.length - 1] = e;
          continue;
        }
      }
      deduped.push(e);
  ...
  return { generate, assess, bestArmorLineups, BR_WINDOW, PLAYSTYLES, SPADED };
})();
